...
4. **REST provider identity (REMAINING GATE — direct change NOT viable, verified 2026-08-01)** — the S4U2Proxy ST for `HTTP/mbr02.range.local` is encrypted with **svc_sccm's key**. The REST provider runs as **LocalSystem** (inside `SMS_EXECUTIVE`) and **cannot decrypt it → 401** even with a valid ST (verified live: `getST` issued an `administrator@HTTP_mbr02.range.local` ST, presentation → **401**).

    > ⚠️ **ATTEMPTED + REJECTED (2026-08-01):** switching `SMS_EXECUTIVE` to `RANGE\svc_sccm` via `sc config` **broke the site** (503/DOWN — smsexec cannot initialize components as a plain domain user; SCCM only provisions the required log/registry/permission rights via its Setup wizard). Revert with `sc config SMS_EXECUTIVE obj= "LocalSystem" password= ""` **FAILED SILENTLY** (the `password=` arg invalidates the LocalSystem config). Correct revert = `sc config SMS_EXECUTIVE obj= "LocalSystem"` (NO password arg) → restart → site recovered (REST provider running, endpoint back to 401).
    >
    > **Conclusion:** the ONLY viable way to run the REST provider as `svc_sccm` is **SCCM Setup → site maintenance → modify the site system installation account** (GUI wizard; provisions all permissions automatically). **Operator action required** — this cannot be scripted safely. Until then the CD ST can't be consumed → WT037/039 remain gated. Artifacts from the attempt (kept, harmless): `RANGE\administrator` in `SMS Admins`, `SeServiceLogonRight` for `svc_sccm`, `RANGE\svc_sccm` SQL login on the CAS database.
