...
**Current Session (2026-08-03 — Campaign H initial access set up + 5/6 verified live; 3.5G DPAPI VERIFIED end-to-end via CRTE tools):**
- **CRTE tools merged (2026-08-03):** user copied CRTE course tools to ws01 `C:\Tools\CRTE_Tools` → consolidated to `C:\Tools\ADTools\CRTE-2026\` (dup tree removed). Unique/newer tools: **SharpDPAPI (2026-02-02 build)**, Rubeus (2026-02-16), DCOMRunAs, Keycred, chromelevator_x64, SharpADWS, SOAPHound, SharpGPOAbuse, PPLBlade (deobfuscate.py + pypykatz.exe), DSInternals_v5.3, EntraConnectAbuse, InviShell, Powermad.
- **3.5G → ✅ VERIFIED end-to-end (2026-08-03):** the CRTE SharpDPAPI **imports our cadre domain DPAPI PVK without `Bad Version of provider`** — blocker was tool-version, not platform. Full chain on dc01 (WinRM as DA): backup key → masterkeys decrypted (Administrator `{07024635…}:624FE5B4…`, chief_command `{9cdd94c1…}:094F1372…`) → **real blobs decrypted** (Administrator Credential `DFBE70A7…` + Web Credentials Vault aes128/aes256 keys). Rollup: **119 = 83 ✅ / 9 ⚠️ / 10 ⏳ / 11 🔬 / 6 ❌**.

**Current Session (2026-08-03 — Campaign H initial access set up + 5/6 verified live; H-01..H-06 staging complete):**
- **Source / scope:** Per user "setup, review and tell me" for Campaign H (WT063-068) + topology correction ("this one require ws01 as a target and provisioning exploiting for initial access" — the ONLY branch where provisioning = attacker, ws01 = target). Built the full delivery stack + validated live.
- **Topology:** attacker = **provisioning** (`192.168.77.60`, HTTP `:8081` → `~/www`); target = **ws01** (`analyst_t1`, Windows 11 + MDE). All artifacts hosted + serving HTTP 200: `Invoice.lnk` (H-01), `H-02-evil.msi` (H-02), `H-03-evil.chm` (H-03), `H-04-smuggle.html` (H-04), `payload.exe` (H-06), `AutoIt3.exe` (H-05).
- **Verified live (marker `H-PAYLOAD|executed as CHILD\analyst_t1|WS01`):** **H-01 LNK ✅** (Invoice.lnk 1793B, click → exec) · **H-02 MSI ✅** (WiX candle+light → 32768B MSI → `msiexec /i /qn` → deferred CA `After=InstallFiles` — ICE77 sequencing fix — ran payload) · **H-05 AutoIt3 ✅** (portable AutoIt3.exe 980064B + au3 from :8081) · **H-06 EXE ✅** (payload.exe 4096B from :8081). **H-04 ✅ builder** (wt066-html-smuggling.py → H-04-smuggle.html 5883B, payload embedded; browser detonation = user practice per Rule 3). **H-03 ⚠️ build+content verified** (hhc compiled 9306B; decompile confirms Shortcut object + `cmd.exe /c payload.exe`) but **execution blocked by modern hh.exe ActiveX sandbox** — platform limit, same class as WT012 Rubeus PAC.
- **Tooling (staged on ws01 `C:\Tools\campaign-h\www\`, no admin):** WiX `wix\candle.exe`+`light.exe` (wix314-binaries.zip), HTML Help Workshop `hhw\hhc.exe` (htmlhelp.exe extracted via 7-Zip: 7zr.exe + 7z2602-extra.7z → 7za.exe → `7za x htmlhelp.exe`; MS live URL serves 1213B error → web.archive.org snapshot), AutoIt3.exe portable, 7-Zip. H builder scripts: `attack-matrix/04-automation/campaign-h/wt063-068*` (made REAL — WiX/CHM/AutoIt builders + markers). Orchestrator + helpers: `attack-matrix/04-automation/linux/windows/wt-h-*` (recon/setup/finish/hhw-extract 1-3/check/run-all/02-03/artifacts).
- **New playbooks:** `ansible/playbooks/19-initial-access.yml` + `-verifyOnly.yml` — provisioning `~/www` staging + idempotent `http.server :8081` (nohup, `~/www-server.log`) + ws01 staging dir; verify = artifact presence/sizes + HTTP 200 ×6 + :8081 listening. Artifacts committed under `ansible/files/campaign-h/` (payload.exe, Invoice.lnk, H-02-evil.msi, H-03-evil.chm, H-04-smuggle.html). `cadre.py` install loop stops at 15 — 19 is standalone like 16/17/18 (no registration needed).
- **Files updated:** `CAMPAIGNS-VALIDATION-REPORT.md` (H row → 5 ✅/1 ⚠️; rollup **119 = 82 ✅ / 10 ⚠️ / 10 ⏳ / 11 🔬 / 6 ❌**), `CAMPAIGNS_v3.md` (Phase 0.5 validation banner + per-H status), `CAMPAIGNS-METADATA-v2.md` (Phase 0.5 statuses + playbook/source-machine corrections), `CHANGELOG.md`, `ACTIVE.md`, `AGENTS.md`.
- **Next:** **RedStrike run of the H/campaign scenario** (user: "remember we have to try with redstrike next. And i expect RedStrike to do better than your scripted approach") — RedStrike live on provisioning (`~/RedStrike`), `--execute` operator-gated (HITL). Then Post-DA exec (DPAPI-NG target · DLL-hijack app · SSP DLL) → Plan 1 telemetry catalog.

**Current Session (2026-08-02 — Branch C COMPLETE: WT037 CMPivot + WT038 app deploy + WT039 script-as-SYSTEM ALL FULL EXEC VERIFIED — SCCM blocker CLOSED):**
- **Source / scope:** Finish Branch C full execution per user direction ("Do it, once it works we update all the docs"). Reference research: mayfly277 *SCCM-LAB part0x0-0x3* + `garrettfoster13/sccmhunter` source (exact REST recipes). Result: **WT037 CMPivot ✅, WT038 application deployment as SYSTEM ✅, WT039 arbitrary script as `NT AUTHORITY\SYSTEM` ✅ on the WS01 managed client (ResourceID 16777220), all from ws01 as `range\svc_sccm` (WT033 identity).**
- **Three stacked real root causes (each masked the next):**
  1. **BGB fast channel dead** — `C:\Program Files\SMS_CCM\SMS_BGB` (IIS `/bgb` vdir) was EMPTY (client cleanup deleted it; MP re-add doesn't restore) → `/bgb/handler.ashx?RequestType=LogIn` → HTTP 500 (`0x87d0027e`) → ClientOperations never dispatched (State=1 forever). **Fix:** installed `cd.latest\SMSSETUP\BIN\X64\bgbisapi.msi` (SITECODE=CAD SMSDIR=...) + `Restart-Service SMS_EXECUTIVE` → **TCP 10123 LISTENING**, WS01 `CcmNotificationAgent.log`: "Receive signin confirmation message from server" + `192.168.77.23:10123 ESTABLISHED`.
  2. **`svc_sccm` was never an RBAC SCCM admin** — only `MBR02\vagrant` in `RBAC_Admins` (SMS Admins local group = namespace only, not RBAC; Phase 7.5 `SMS_Admin` creation never persisted). **Fix:** Takeover-1 DB primitive — `INSERT INTO RBAC_Admins` + `RBAC_ExtendedPermissions` mirroring admin 16777217 (`SMS0001R`, scopes `SMS00001`/1, `SMS00004`/1, `SMS00ALL`/29) → restart SMS_EXECUTIVE → Device read 403→200.
  3. **Script approval blocked** — `UpdateApprovalState` → 500 (author can't self-approve) → **`UPDATE Scripts SET ApprovalState=3`** (table is `Scripts`, we hold `sa`).
- **WT038 (added this session):** app created via AdminService `POST /AdminService/wmi/SMS_Application` with SCCMHunter-exact AppMgmtDigest (app 16777510 + DT 16777511 + assignment 16777217 + policy body). Two model gotchas: `<CustomData>` + `<ExecutionContext>` at `<Installer>` level required; **XML-escape the `InstallCommandLine` payload** (`>`/`&` break the parser). **Delivery blocker root-caused:** MP web handlers were EMPTY (`SMS_MP` + `ServiceData\System` → `/SMS_MP/.sms_aut` 500 → client `/ccm_system/request` 0x8000000A → no policy) — **Fix:** `msiexec /i ...\mp.msi REINSTALL=ALL REINSTALLMODE=vmaus ...` (exact site cmd from `MPSetup.log`) → health 200 → client delivered → `AppEnforce.log` ran payload with system context → `wt038-system.txt` = `nt authority\system` + `wt038-marker.txt` on WS01. GUI collection schedule (incremental) also required for programmatic collections (guide Phase 6C).
- **Verified results:** WT037 `RunCMPivot` (LogicalDisk) → live data `{"Result":[{"DeviceID":"C:","FileSystem":"NTFS","FreeSpace":"153601","Size":"203674","SystemName":"WS01",...}]}`. WT039: create via `POST /AdminService/wmi/SMS_Scripts.CreateScripts/` (base64 UTF-16LE+BOM, ScriptType 0, caller GUID) → `RunScript` → `ScriptResult` → `ScriptOutput: "nt authority\system"` + `C:\Windows\Temp\wt039-system.txt`/`wt039-marker.txt` on WS01.
- **Tooling/notes:** ws01 python 3.12.8 + impacket 0.13.1 + pyodbc 5.3.0 (`pip install pyodbc`); SCCM DB `CM_CAD` via `sa:s@_P@ssw0rd!L@b!` (Driver={SQL Server}); SSH to mbr02 as vagrant via `cadre-provisioning-key`. Raw WMI `SMS_Scripts.CreateScripts` fails ("Property (Feature) Missing!") — use the AdminService wmi passthrough. Reference tool = SCCMHunter (`sccmhunter.py admin -u ...` → `interact` → `script /path.ps1`).
- **Files updated:** `docs/sccm-integration-guide.md` (**Phase 6B/6C/6C.4** — recipes + 3 fixes + MP repair), `CAMPAIGNS_v3.md`, `CAMPAIGNS-METADATA-v2.md`, `CAMPAIGNS-VALIDATION-REPORT.md`, `CADRE-Attack-Surface-Coverage-Audit.md`, `CHANGELOG.md`, `AGENTS.md`; new scripts under `attack-matrix/04-automation/linux/sccm/` (ws01-wt037*, ws01-wt038*, ws01-wt039*, ws01-add-svc-admin.ps1, ws01-sql-*, mbr02-bgb*, mbr02-wt038-*). Docs renamed per user direction: `CAMPAIGNS-VALIDATION-REPORT-20260730.md` → `CAMPAIGNS-VALIDATION-REPORT.md`, `CADRE-Attack-Surface-Coverage-Audit-20260730.md` → `CADRE-Attack-Surface-Coverage-Audit.md`.
- **Next:** WT035 PXE (needs real PXE client), WT036 client-push relay (needs console-created device). Then commit + proceed to plan1 telemetry catalog (per user: core lab + branches gated before plan1).

**Current Session (2026-08-01 — Branch D WT047 completed + Branch C deep validation):**
- **Source / scope:** Finish Branch D properly (WT047 NFS krb5p) per user direction, then Branch C (SCCM WT035-039) validation via the D → C → B reverse order. Propagate all config fixes to deploy/verifyOnly playbooks + integration guides (user rule: automatable = playbook, manual = guide).
- **Branch D (WT047) COMPLETE:** Root-caused + fixed live: (1) `rpc-svcgssd` was FAILED (`No key table entry found matching nfs/`) → registered `nfs/linux01` + `nfs/linux01.cadre.local` SPNs (ADSI + `adcli update --add-service-principal`) + enabled `rpc-svcgssd` (Ubuntu unit, NOT `nfs-secure-server`); (2) `/etc/idmapd.conf` Domain=`localdomain` → `cadre.local`; (3) mount **by FQDN** not localhost. Result: `mount -t nfs4 -o sec=krb5p linux01.cadre.local:/exports/secure-share /mnt/cadre-nfs` → **MOUNT_OK + read** (write denied = root-owned 0755 share dir). All 5 Branch D WTs now ✅ (044 entry, 045 SSSD, 046 keytab, 047 NFS krb5p, 048 podman escape). Fixes propagated: `07-linux-config.yml` (idmapd, nfs principal rc-convention task, rpc-svcgssd, gssd restarts), `07-linux-config-verifyOnly.yml` (idmapd/svcgssd/nfs-principal checks), `sql-integration-guide.md` §3.5 (NFS krb5p recipe).
- **Branch C deep validation (WT034-039):** SharpSCCM v2.0.13 staged on ws01. SMS Provider (`root\SMS\site_CAD`) reachable as `range\svc_sccm` via **WMI explicit creds** (no Kerberos needed — `/ptt` + cmdkey both blocked by OpenSSH logon-session limits). Verified: local `SMS Admins` membership (svc_sccm + **cross-forest `CADRE\chief_command`/`analyst_purple`**), NAA=RANGE\svc_naa in machine-policy config, PXE **approved cert** (`{256B7D4F-…}`, MBR02) + 2 boot images, client-push component enabled, `SMS_Scripts.CreateScripts` works (caller GUID required), `SMS_Package`+`SMS_Program` (SYSTEM) creation works. **Root-caused blocker:** **AdminService NOT deployed on mbr02** (no IIS app pool, no `/AdminService` web app, no `bin\AdminService`) → CMPivot (WT037) / script-run (WT039) have no target; the `/AdminService/` 401 was IIS-level, not auth. The `HTTP/mbr02.range.local` SPN on `svc_sccm` + `msDS-AllowedToDelegateTo=HTTP/mbr02.range.local` (intentional per `05-ad-attack-surface.yml`) is **correct**; intended path = post-Kerberoast **S4U2Self→S4U2Proxy to HTTP/mbr02 → AdminService**, with the AdminService **app pool running as `RANGE\svc_sccm`** (SPN owner, required to decrypt CD tickets). Fix documented in `docs/sccm-integration-guide.md` **Phase 6A** + verify checks in `10-sccm-verify.yml`. (2) `SMS_Advertisement.Put` = Generic failure via raw WMI (console needed) → WT038/039 full exec via console/CD. (3) **`cifs/mbr02.range.local` SPN missing** → SMB Kerberos to mbr02 broken (verify-playbook candidate; not changed — may be intentional relay surface). WT036 coercion primitives (`GenerateCCRByName`/`CreateCCR`) invoked (needs console-created target device for full relay). Test objects cleaned up; ntlmrelayx stopped + smbd restored on provisioning.
- **Files updated:** `attack-matrix/Campaign/CAMPAIGNS-VALIDATION-REPORT.md`, `CAMPAIGNS_v3.md`, `CAMPAIGNS-METADATA-v2.md`, `Runbooks/CAMPAIGNS-RUNBOOK-branch-c.md`, `docs/sql-integration-guide.md`, `docs/sccm-integration-guide.md`, `ansible/playbooks/10-sccm-verify.yml`, `ansible/playbooks/07-linux-config.yml`, `ansible/playbooks/07-linux-config-verifyOnly.yml`, `CHANGELOG.md`, `AGENTS.md` + new automation scripts under `attack-matrix/04-automation/linux/windows/` (branchc-*).
- **Branch B ADCS — ESC2/ESC4/ESC7/ESC9 VERIFIED (2026-08-01).** Fresh `certipy find -vulnerable` confirmed surface (CA cadre-CA ESC7/ESC8/ESC11; **CADRE-ESC13/14 not deployed**; ESC6 not deployed). Validated as low-priv `hunter_dfir`: **ESC2** (CADRE-ESC2 Any Purpose, Req 45) → PKINIT TGT admin; **ESC9** (CADRE-ESC9 NoSecurityExtension, Req 46) → PKINIT + UnPAC NT hash `81c3b644…f1eb7b`. **ESC4** (CADRE-ESC4) as `lead_engineering` → `certipy template -write-default-configuration` → enroll admin (Req 47) → NT hash; **template restored** (NameFlag verified back to original). **ESC7** as `lead_engineering` → `certipy ca -add-officer` succeeded (ManageCA proof) + cleaned. ESC11 deferred with ESC8 (relay family). Artifacts cleaned.
- **Next:** Branch B ESC8 revisit (deferred) or next ESC item; then plan1 telemetry catalog.

**Previous Session (2026-07-30 — Branch A, C, D final verification + playbook fix):**
- **Source / scope:** Complete live testing of Branch A (ACL Abuse / ACE#7), Branch C (SCCM NAA extraction), Branch D (Linux MSSQL linked-server pivot), and T102 dc02 Print Spooler prerequisites. Update campaign docs, metadata, seed files, and vulnerability verify-only/deploy playbooks.
- **What was done:**
  ...
  - **T102 (dc02 Spooler):** `04-vulnerabilities-verifyOnly.yml` initially failed because `RunAsPPL=0` was treated as failure, and Spooler firewall verification missed exact rule names. Fixed verify-only to pass on `RunAsPPL=0`; added validated SMB/RPC firewall verification fallback. Updated deploy playbook to explicitly create `CADRE-Spooler-SMB-In` TCP/445, `CADRE-Spooler-RPC-In` TCP/135, and `CADRE-Spooler-RPC-Dynamic-In` TCP/49152-65535 on `dc02`. Re-ran deploy + verify-only: deploy applied cleanly, verify-only passed `ok=30 changed=0 failed=0 skipped=0`. Campaign/docs metadata moved T102 from hard `BLOCKED` to `Trigger verified / capture pending` pending Rubeus ticket capture validation.
  ...
  - **Docs updated:** `CAMPAIGNS-METADATA-v2.md`, `CAMPAIGNS_v3.md` (ASCII diagram + Branch A/C/D sections + credential tables + Phase 5 coercion status), `CAMPAIGNS-VALIDATION-REPORT.md`, `CADRE-Attack-Surface-Coverage-Audit.md`, `CHANGELOG.md`, `AGENTS.md`.
- **Files updated:** `attack-matrix/Campaign/CAMPAIGNS-METADATA-v2.md`, `attack-matrix/Campaign/CAMPAIGNS_v3.md`, `attack-matrix/Campaign/CAMPAIGNS-VALIDATION-REPORT.md`, `attack-matrix/Campaign/CADRE-Attack-Surface-Coverage-Audit.md`, `attack-matrix/Campaign/automation/lab-seed-creds.json`, `ansible/files/cadre_passwords.txt`, `ansible/playbooks/05-ad-attack-surface.yml`, `ansible/playbooks/04-vulnerabilities.yml`, `ansible/playbooks/04-vulnerabilities-verifyOnly.yml`, `CHANGELOG.md`, `AGENTS.md`.
- **Current blockers:** Phase 5 T102 coercion remains `Trigger verified / capture pending`; Rubeus ticket capture still needs re-test. WT035-039 SCCM chain deep-validated 2026-08-01 (full exec ⚠️ console/AdminService-gated — see current session). Skipjack ⏳ Pending custom PoC.
