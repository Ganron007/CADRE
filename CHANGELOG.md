...
- **3.5G DPAPI → ✅ VERIFIED end-to-end on Server 2025 (CRTE-informed retry, 2026-08-03).** Per user, pulled the CRTE course tools (`C:\Tools\CRTE_Tools` → consolidated to ws01 `C:\Tools\ADTools\CRTE-2026\`, duplicate 19,535-file tree removed). Key win: the CRTE **SharpDPAPI build (2026-02-02, 476KB) imports our cadre domain DPAPI PVK without the `Bad Version of provider` error** that blocked the 2026-08-03 run — the blocker was a **tool-version issue, not a Server 2025 platform quirk**. Full chain verified live on **dc01** (WinRM as DA `chief_command`): backup key → **masterkeys decrypted** (`Administrator` `{07024635…}:624FE5B4…`, `chief_command` `{9cdd94c1…}:094F1372…`) → **real blobs decrypted**: Administrator Credential (`DFBE70A7…` WindowsLive PersistedCredential) + **Web Credentials Vault** (recovered `aes128`/`aes256` vault keys). Also merged CRTE unique tools: Rubeus (2026-02-16 build), DCOMRunAs (WT108), Keycred (shadow creds), chromelevator_x64 (Chrome App-Bound), SharpADWS, SOAPHound, SharpGPOAbuse, PPLBlade (deobfuscate.py + pypykatz.exe), DSInternals_v5.3, EntraConnectAbuse. Rollup: **119 = 83 ✅ / 9 ⚠️ / 10 ⏳ / 11 🔬 / 6 ❌**. Docs synced: validation report, metadata-v2, CHECKLIST, CHANGELOG, ACTIVE, AGENTS.
