...
## [Unreleased] — 2026-08-03 (ESC8 krbrelayx + RedStrike 0.5.1)

- **ESC8 / krbrelayx revisit (WT052).** Live from ws01: staged `krbrelayx` + `krbrelayx-venv` (`impacket==0.12.0`), dnstool LDAPS DNS record `esc8relay.cadre.local` → ws01, HTTP-only krbrelayx patch (kernel holds :445). **Listener :80 verified**; **no cert issued** — coercion does not deliver Kerberos HTTP to ws01 (same relay-family blocker as NTLM). Scripts: `esc8-krbrelayx-chain.ps1`, `esc8-krbrelayx-coerce.ps1`, `esc8-krbrelayx-patch.ps1`, `esc8-krbrelayx-run.ps1`. Validation report + root-cause record updated; WT052 remains 🔬.
- **RedStrike 0.5.1** (sister `RedStrike\` + pin `tools/red-strike/`): `ws01_transport.py` — typed intents SSH-wrap to ws01; graph **v8** (branch H stubs, Post-DA T097–109 stubs, `certipy.find`, Branch D linux scripts, T102 intent removed). 91 pytest. `red-strike.md` campaign-ready checklist updated.

## [Unreleased] — 2026-08-03

- **Attack validation closure pass (satisfactory bar).** Rollup **119 = 84 ✅ / 10 ⚠️ / 0 ⏳ / 19 🔬 / 6 ❌** — attack-side `⏳` cleared. **WT099 ✅** (range KDS 64B + dMSA SID + `msDS-ManagedPasswordId` via `wt099-dmsa-prereq.ps1` on ws01). **3.5C ✅** prereqs (`wt035c-rdp-prereq.ps1`: :3389 + SMB IPC$ auth). Reclassified 🔬: WT103 (no DPAPI-NG blob), WT104/107 (staging), WT096/108 (tooling), NTLMv1/3.5L, Branch G CVE. Published satisfactory bar + RedStrike campaign-ready checklist in validation report + `red-strike.md`. **Next:** operator `--execute` smoke → Plan 1 telemetry.

## [Unreleased] — 2026-08-03 Consolidated updated tool builds on ws01 → `C:\Tools\ADTools\tools-2026\` (duplicate tree removed). Key win: the updated **SharpDPAPI build (2026-02-02, 476KB) imports our cadre domain DPAPI PVK without the `Bad Version of provider` error** that blocked the earlier 2026-08-03 run — the blocker was a **tool-version issue, not a Server 2025 platform quirk**. Full chain verified live on **dc01** (WinRM as DA `chief_command`): backup key → **masterkeys decrypted** (`Administrator` `{07024635…}:624FE5B4…`, `chief_command` `{9cdd94c1…}:094F1372…`) → **real blobs decrypted**: Administrator Credential (`DFBE70A7…` WindowsLive PersistedCredential) + **Web Credentials Vault** (recovered `aes128`/`aes256` vault keys). Also merged unique tool builds: Rubeus (2026-02-16), DCOMRunAs (WT108), Keycred (shadow creds), chromelevator_x64 (Chrome App-Bound), SharpADWS, SOAPHound, SharpGPOAbuse, PPLBlade (deobfuscate.py + pypykatz.exe), DSInternals_v5.3, EntraConnectAbuse. Rollup: **119 = 82 ✅ / 10 ⚠️ / 10 ⏳ / 11 🔬 / 6 ❌**. Docs synced: validation report, metadata-v2, CHECKLIST, CHANGELOG, ACTIVE, AGENTS.

## [Unreleased] — 2026-08-03

- **Campaign H (WT063-068) — initial-access stack set up + 5/6 vectors verified live (2026-08-03).** Per user ("setup, review and tell me" + topology correction: "this one require ws01 as a target and provisioning exploiting for initial access"). Attacker = **provisioning** (`192.168.77.60`, HTTP `:8081` → `~/www`), target = **ws01** (`analyst_t1`). Full delivery chain verified: artifacts hosted on provisioning:8081 (all HTTP 200), ws01 downloads + executes. **Verified live:** **H-01 LNK ✅** (Invoice.lnk 1793B → marker `H-PAYLOAD|executed as CHILD\analyst_t1|WS01`), **H-02 MSI ✅** (WiX candle+light → 32768B MSI → `msiexec /i /qn` → deferred CA `After=InstallFiles` [ICE77 fix] ran payload → marker), **H-05 AutoIt3 ✅** (AutoIt3.exe 980064B + au3 from :8081 → marker), **H-06 EXE ✅** (payload.exe 4096B from :8081 → marker), **H-04 HTML smuggling ✅ builder** (wt066-html-smuggling.py → H-04-smuggle.html 5883B, payload embedded). **H-03 CHM ⚠️ build+content verified** (hhc compiled 9306B; decompile confirms Shortcut object + `cmd.exe /c payload.exe` embedded) but execution blocked by **modern hh.exe ActiveX sandbox** (platform limit — same class as WT012 Rubeus PAC). Tooling staged: WiX (wix314-binaries), HTML Help Workshop (hhc via htmlhelp.exe + 7-Zip, no admin), AutoIt3 portable, all on ws01 `C:\Tools\campaign-h\www\`. H-01..06 scripts made REAL (`attack-matrix/04-automation/campaign-h/wt063-068*`), new orchestrator + H tooling (`attack-matrix/04-automation/linux/windows/wt-h-*`: recon/setup/finish/hhw-extract/run-all/02-03/artifacts). **New playbooks:** `ansible/playbooks/19-initial-access.yml` + `-verifyOnly.yml` (stages H artifacts on provisioning `~/www`, idempotent http.server :8081, ws01 staging dir; verify checks presence/sizes/HTTP 200). Artifacts committed under `ansible/files/campaign-h/` (payload.exe, Invoice.lnk, H-02-evil.msi, H-03-evil.chm, H-04-smuggle.html). Docs synced: `CAMPAIGNS-VALIDATION-REPORT.md` (H row 6 pending → 5 ✅/1 ⚠️; rollup **119 = 82 ✅ / 10 ⚠️ / 10 ⏳ / 11 🔬 / 6 ❌**), `CAMPAIGNS_v3.md` (Phase 0.5 validation banner + per-H status + **full runbook expansion**: shared delivery chain + build/deliver/execute/verify steps per vector), `CAMPAIGNS-METADATA-v2.md` (Phase 0.5 statuses + playbook/source-machine corrections), `CHECKLIST.md` (C.0.5/C.H rows + H summary + Appendix A row + 3.5 sync), `CADRE-Attack-Surface-Coverage-Audit.md` (Phase 0.5 rows + §2.1 + §3.6 DONE + P1 DONE), `CHANGELOG.md`, `ACTIVE.md`, `AGENTS.md`. **Next:** RedStrike run of H/campaign scenario (user expects RedStrike to beat the scripted approach).

## [Unreleased] — 2026-08-03

- **Push-2: complete the 3 blocked items (2026-08-03).** Per user challenge, pushed the 3 partials I'd stopped short on. **3.5K → ✅ VERIFIED (extraction)** — downloaded procdump v12.01 (live.sysinternals) + community Rubeus 2.2.0 (Ghostpack-CompiledBinaries mirror; GhostPack ships source-only). `procdump -ma lsass` → **62MB real full dump** (comsvcs 64-76KB stubs were tool-specific, not env); `Rubeus dump` live-extracted LSASS logon sessions + tickets: **analyst_cloud (CADRE), MBR01$, MSSQL$SQLEXPRESS, cross-domain CADRE.LOCAL tickets**. Caveats recorded: mimikatz 2.2.0 + pypykatz 0.3.15 can't parse Server 2025 LSASS; WerFault `-u -p -ip` no dump. **3.5G → ⚠️ upgraded (chain 80%)** — domain DPAPI backup key EXTRACTED via mimikatz `lsadump::backupkeys /export` on dc01 (WinRM as chief_command): RSA PVK + legacy 256B key + DER + PFX (`ntds_capi_0_73eeb965…`, `ntds_legacy_0_71f6589e…`); SharpDPAPI masterkeys ENUMERATED all 4 profiles (**analyst_cloud.CADRE** → `6a912b23…`); decryption blocked by SharpDPAPI 1.12.0 + latest `Bad Version of provider` on key import (Server 2025 CSP/CryptoAPI quirk). Reopen: latest pypykatz/Nemesis. **WT012 → ⚠️ upgraded (tool-compat)** — real Rubeus 2.2.0 (readable stack): legit TGT ✅ (AS-REQ), krbtgt AES256 ✅, service SPN ✅; fork = `Unable to decrypt ticket or get PAC` (Rubeus 2.2.0 PAC parser vs Server 2025 KDC); `/tgtdeleg` arg-bug + SSH limitation. No newer prebuilt exists. Rollup: **119 = 77 ✅ / 9 ⚠️ / 16 ⏳ / 11 🔬 / 6 ❌**. New automation: `wt035k-v2..v6`, `wt035g-step1..step3`, `wt012-v2..v5`, `wt-download-tools`, `wt-tools2`, `wt-rubeus-*`, `wt-sdp-dl`, `wt-recon3`, `wt-show-bk`, `wt-cat2`, `wt-pypykatz-dl`.

## [Unreleased] — 2026-08-03

- **Testable-now batch 2 — live validation (2026-08-03).** Ran the "testable now" batch from ws01 (Rule 1). **WT011 Silver ✅ VERIFIED** — forged `cifs/mbr01.child.cadre.local` silver (MBR01$ NT `3a01c6cd…`, Administrator id:500 + group 512) via mimikatz `kerberos::golden /service`; **real-service proof**: impacket `smbclient.py -k` → `use c$` listed full mbr01 C$ (no KDC contact); Windows `dir` client silently falls back to NTLM (pre-existing session) — explicit-ticket verify needs `-k`. **3.5D File detonation ✅** — SYSTEM dropped marker to `C:\Users\analyst_cloud\Downloads\` (content verified, cleaned); `quser` confirms analyst_cloud ACTIVE console session 1. **3.5G ⚠️** — SharpDPAPI 1.12.0 runs as SYSTEM; `masterkeys` gated on domain DPAPI backup key (`/pvk` etc.); Nemesis not staged. **3.5H ⚠️** — ctfmon running in analyst_cloud session 1; comsvcs dump = 64KB stub (env); procdump not staged. **3.5J ⚠️** — MOF compile creates filter/consumer/binding (5861 activation) BUT permanent-sub delivery never reaches consumer on Server 2025: `WITHIN` rejected (0x80041017 Invalid query), `TargetInstance ISA 'Win32_Process'` WHERE never matches, bare `__InstanceCreationEvent` works as TEMP sub only. Original staged script (WITHIN) was broken in this env; MOF is the correct creation primitive (Set-WmiInstance/CIM binding both fail). **3.5K ⚠️** — PPL=0, SeDebugPrivilege OK, but comsvcs→76KB stub, WerFault `-u -p -ip`→no dump, mimikatz 2.2.0 sekurlsa→`kuhl_m_sekurlsa_acquireLSA` (build too old for Server 2025). **WT012 ⛔ tool-blocked** — staged Rubeus is an obfuscated build whose `diamond` action is a stub (ignores `/service`/`/dc`, hardcodes `cifs/dc.domain.com`, `/tgtdeleg` fails over SSH); prereqs validated (legit TGT ✅, krbtgt AES256 ✅); reopen with official Rubeus. **3.5M 🔬 deferred** — AAD Connect NOT deployed (no `ADSync` on dc01). **WT096/WT108 still ⏳** — nxc / DCOMIllusionist not staged on ws01. Rollup: **119 = 76 ✅ / 10 ⚠️ / 16 ⏳ / 11 🔬 / 6 ❌**. New automation under `04-automation/linux/windows/`: `wt011-*`, `wt035d-*`, `wt035g-*`(in combined payload), `wt035h-*`, `wt035j-*`, `wt035k-*`, `wt-mbr01-run.ps1` (generic payload runner), `wt-cat.ps1`.

## [Unreleased] — 2026-08-02

- **Post-DA cluster — first validation batch (2026-08-03).** Live from ws01 (Rule 1 direct SSH) with DA context. **WT097 ✅** KDS Root Key — 2 × 64-byte root-key blobs via LDAPS (`ec8f491f-…`, `877a6c10-…`; SP800_108_CTR_HMAC, DH 512/2048); `Get-KdsRootKey` cmdlet returns null RootKeyData in this env (direct LDAP is the reliable path). **WT098 ✅ (prereqs, Rule 3)** — KDS key `877a6c10-…` (from pwdid) + gMSA SID + msDS-ManagedPasswordId (L0/L1/L2); offline compute = user practice (GoldenGMSA 1.0.1.0 expects full `KDS_ROOT_KEY`, lab stores a 64-byte blob → pyGoldenGMSA / SP800-108). **WT100 🔬 DEFERRED** — LAPS NOT implemented (no `ms-Mcs-AdmPwd` schema, no `msLAPS-Password`, no playbook) → future suggestion. **WT101 ✅ (extraction)** — DSRM `Administrator:500` hash `81c3b6443f148bf73bb3499791f1eb7b` (= cadre.local Administrator, cross-validated via ESC1 UnPAC); `DsrmAdminLogonBehavior` absent (default blocked); SET = user practice. **WT102 ⛔ BLOCKED** — mimikatz dcshadow "computer not found in AD 0x1" (env) in all contexts; prereqs validated (EA, child/krbtgt `b6c370f2…`, child SID). **WT105 ✅ COM hijack** (SYSTEM plants `HKCU\Software\Classes\CLSID\{…}\InprocServer32` → attacker DLL, verified + cleaned). **WT106 ✅ IFEO** (Debugger → notepad → marker `WT106-IFEO`, cleaned). **WT109 ✅ ESC16** — `DisableExtensionList` absent on cadre-CA → SID ext enabled → not configured (default). **Rule 3 added to `docs/internal/cadre-lab-contract.md`** — no password cracking/brute-force in validation; extraction + prerequisites = VERIFIED, computation/mutation = user practice. Rollup: **119 = 74 ✅ / 7 ⚠️ / 22 ⏳ / 10 🔬 / 6 ❌**. New automation: `04-automation/linux/windows/wt097-*`, `wt098-*`, `wt100-*`, `wt102-*`, `wt105-*`, `wt106-*`.
- **Campaign suggestions — ShadowDumper LSASS-dump item added (Item #116, 2026-08-02).** PracSec 2026-06-29 article "Dumping LSASS Without Touching Disk: Improvements to ShadowDumper" + Offensive-Panda's **ShadowDumper** added to `Campaign_suggestions.md` (Phase 3.5 row, ⏳) + full write-up in `archive/Campaign_suggestions-detail.md`: in-memory callback dump (`S_FALSE` + `DELETE_ON_CLOSE` throwaway handle), syscalls resolved from on-disk `ntdll.dll` (beats EDR inline hooks), hand-built MDMP without `dbghelp.dll`, GZip+XOR with per-run random key (defeats Defender `Trojan:Win32/LsassDump.A`), no hardcoded paths, in-memory per-SSP credential extraction (MSV1_0/WDigest/Kerberos/TSpkg/SSP/LiveSSP/CredMan/CloudAP/DPAPI/RDP). Phase 3.5 alternative to verified `3.5F`/`3.5F-alt` + Track A (hardened variant) candidate; detection engineering held for plan1.7 §17. Counts 110 → 111 (⏳ 45).
- **Campaign doc restructure + stale-total review (2026-08-02).** Post-adoption review pass found the flow diagrams and counts were stale — all reconciled: `CAMPAIGNS_v3.md` + `CAMPAIGNS.md` headers **81/105 → 94/118**; **ASCII + mermaid flow diagrams now include the Post-DA cluster** (P7 → PD → P8); `tools/split-campaign-runbooks.py` index template total fixed (was silently reverting to 81/105 on regen); `CAMPAIGNS-VALIDATION-REPORT.md` Summary Statistics fixed (Phase 3 2→3, Phase 3.5 14→18, Branch B 8→9; block sum 113→119 matching the rollup); `CAMPAIGNS-METADATA-v2.md` scope line 81/107+ → **94/121**. Runbooks regenerated (coverage OK). **Flow diagram moved out of v3** → new [`CAMPAIGNv3-ATTACK-FLOW.md`](attack-matrix/Campaign/CAMPAIGNv3-ATTACK-FLOW.md) (v3 top stays light; renamed from "CADRE CAMPAIGN v3 — COMPLETE ATTACK FLOW.md"). **Root `CAMPAIGNS.md` index archived** → `archive/CAMPAIGNS.md` (replaced the stale 81/105 copy with the current 94/118 index); split tool now writes the index into `archive/` so the campaign root stays clean; v3 header archive label corrected.
- **Rigor contract layer created (2026-08-02, local-only under `docs/internal/`).** (1) `workspace-rigor-contract.md` (local maintainers) — thin **universal** contract applying to all 12 CADRE-Platform projects (evidence-before-claims · no fabrication · multi-stage gates · human-approval boundary · target-as-data · honest reporting · verify-before-agreeing). (2) `cadre-lab-contract.md` (local maintainers) — **CADRE-specific** contract distilled from `PROMPT.md` (modes BUILD/SOLVE/HUNT/VALIDATE, G0–G7 gates, ledgers bound to campaign docs, CADRE false-completion list, doc-update order). (3) `PROMPT.md` (local maintainers) — canonical generic contract moved from `attack-matrix/Campaign/` (local-only). (4) `ACTIVE.md` (local maintainers) — refreshed as the **umbrella sync tracker**: contract-applicability matrix (universal → CADRE → per-repo), per-project status + next-action for all 12 roots, handoff entries 07-29→08-02, C2Stack added, CADRE-Risk corrected to active. **Wiring:** VS Code user-prompt `cadre-workspace.instructions.md` (applyTo `**`, auto-loaded); `AGENTS.md` updated in **CADRE** (commit `b5986a0`) and **DFIR-Nexus** (commit `f009b4e` — also swept in pre-staged REVAMP doc renames, flagged) pointing at the contract layer; new `AGENTS.md` created on disk for Praxis / RedStrike / RevEng / RevAI / DarkAI / Eva7ion / C2Stack / Risk (**local-only** — gitignored by the global `.gitignore_global` rule, left uncommitted per user decision). CADRE-Courses + CADRE-Integrations skipped (read-only corpora).
...
- **WT037 CMPivot + WT039 script-as-SYSTEM FULL EXEC VERIFIED (2026-08-02) — the 2-day SCCM blocker is CLOSED.** Live from ws01 as `range\svc_sccm` (WT033 Kerberoast identity) against the **WS01 managed client** (ResourceID 16777220). Three stacked real root causes fixed in order:
  1. **BGB fast channel was dead.** `C:\Program Files\SMS_CCM\SMS_BGB` (IIS `/bgb` vdir, client-notification) was **EMPTY** (client cleanup deleted it; MP re-add does not restore it) → `/bgb/handler.ashx?RequestType=LogIn` → HTTP 500 (`0x87d0027e`) → CMPivot/scripts never dispatched (ClientOperation stuck State=1). **Fix:** installed `cd.latest\SMSSETUP\BIN\X64\bgbisapi.msi` (SITECODE=CAD SMSDIR=...) + restarted `SMS_EXECUTIVE` → **TCP 10123 listening**, WS01 signed in ("Receive signin confirmation message from server").
  2. **`svc_sccm` was never an RBAC SCCM admin** (only `MBR02\vagrant` in `RBAC_Admins`; the `SMS Admins` local group grants namespace, not RBAC; Phase 7.5's `SMS_Admin` creation never persisted). **Fix:** Takeover-1 DB primitive — `INSERT INTO RBAC_Admins` + `RBAC_ExtendedPermissions` mirroring the working Full Admin (RoleID `SMS0001R`, scopes `SMS00001`/1, `SMS00004`/1, `SMS00ALL`/29) → Device read 403→200 after provider RBAC refresh (restart SMS_EXECUTIVE).
  3. **Script approval blocked** — `UpdateApprovalState` → 500 (hierarchy setting: author can't self-approve; the SCCMHunter-documented wall) → **bypass: `UPDATE Scripts SET ApprovalState=3`** (table is `Scripts`, not `SMS_Scripts`; we hold `sa`).
  - **WT037 verified result:** `RunCMPivot` (LogicalDisk) → `{"Result":[{"DeviceID":"C:","FileSystem":"NTFS","FreeSpace":"153601","Size":"203674","SystemName":"WS01",...}]}` — live client data.
  - **WT039 verified result:** script created via `/AdminService/wmi/SMS_Scripts.CreateScripts/` (base64 UTF-16LE+BOM, ScriptType 0, caller GUID) → `RunScript` → `ScriptResult` → `ScriptOutput: "nt authority\system"` + on-disk markers `C:\Windows\Temp\wt039-system.txt` / `wt039-marker.txt` on WS01.
  - Reference implementation: **SCCMHunter** (`garrettfoster13/sccmhunter`, mayfly277 *SCCM-LAB part0x2/0x3*); script create goes through the **AdminService wmi passthrough**, NOT raw WMI (raw WMI `CreateScripts` → "Property (Feature) Missing! / Invalid input argument (ScriptGuid)").
  - **Docs updated:** `docs/sccm-integration-guide.md` (**Phase 6B** — full recipes + 3 fixes), `CAMPAIGNS_v3.md` (Branch C status), `CAMPAIGNS-METADATA-v2.md` (Branch C status + WT035-039 note + WT037/039 rows), `CAMPAIGNS-VALIDATION-REPORT.md` (WT037/039 rows), `CADRE-Attack-Surface-Coverage-Audit.md` (WT037/039 rows + verdict). New tooling under `attack-matrix/04-automation/linux/sccm/`: `ws01-wt037-cmpivot*`, `ws01-wt039-*`, `ws01-add-svc-admin.ps1`, `ws01-sql-*`, `mbr02-bgb*.ps1`, `ws01-cmpivot-*`.
  - Remaining Branch C: WT035 PXE (needs real PXE client), WT036 client-push relay (needs console-created device), WT038 app-deploy advertisement (needs console/AdminService). Fast channel + admin grant + script approval now all working.
- **SCCM AdminService CD chain CLOSED — SPN owner was the real gate (2026-08-01, VERIFIED).** The 401 was NOT "provider must run as svc_sccm" — the self-hosted AdminService (`SMS_REST_PROVIDER` in `SMS_EXECUTIVE`) **always runs as LocalSystem** and can only decrypt tickets encrypted to the **machine account (`mbr02$`)**. The lab had registered `HTTP/mbr02.range.local` on `svc_sccm` (modeled on a service-account-run site server) → CD ST was svc_sccm-encrypted → 401. **Fix applied + playbook-updated (`05-ad-attack-surface.yml` + `-verifyOnly.yml`):** moved `HTTP/mbr02.range.local` → `mbr02$`; `svc_sccm` keeps a decoy Kerberoast SPN `HTTP/sccm.range.local` (preserves WT033 cross-forest Kerberoast); CD (`msDS-AllowedToDelegateTo` + `TrustedToAuthForDelegation`) unchanged. **Verified live:** `getST` → ST encrypted to `mbr02$` → presented → `AdminService/wmi/SMS_Site` → **200 + valid OData JSON as `administrator`** (anon still 401). **WT037/039 auth gate CLOSED.** Remaining: CMPivot/script-run exec on the only managed client (MBR02). Docs synced: `sccm-integration-guide.md` Phase 6A gate #4, validation report (WT037/039 rows + findings + priority 1), `CAMPAIGNS-METADATA-v2.md`, branch-c runbook, `CAMPAIGNS_v3.md`, coverage audit, `CHECKLIST.md`. New tooling: `sccm-spn-move.ps1/-run.sh` (SPN move), `sccm-spn-getst-present.sh`, `sccm-spn-prove-admin.sh`.
- **SCCM REST-provider identity change attempted + REJECTED — direct `sc config` is NOT viable (2026-08-01).** Per user direction ("Yes do a and finish this"), tried to close the last WT037/039 gate by switching `SMS_EXECUTIVE` to `RANGE\svc_sccm` via `sc config` (provider must run as the SPN owner to decrypt the CD ST). **It broke the site (503/DOWN)** — smsexec cannot initialize components as a plain domain user (SCCM only provisions log/registry/permission rights via its Setup wizard). Revert with `sc config SMS_EXECUTIVE obj= "LocalSystem" password= ""` **FAILED SILENTLY** (the `password=` arg invalidates a LocalSystem config). **Correct revert: `sc config SMS_EXECUTIVE obj= "LocalSystem"` (NO password arg)** → restart → site recovered: `SCCMProviderGraph` running, `SMS_REST_PROVIDER.log` updating, `/AdminService/v1.0/` back to **401**. Re-presented the CD-issued `administrator@HTTP_mbr02.range.local` ST → still **401** (provider as LocalSystem can't decrypt a ticket encrypted to svc_sccm) — confirming the gate. **Conclusion:** the only viable path to run the REST provider as `svc_sccm` is **SCCM Setup → site maintenance → modify site system installation account (GUI wizard, operator-run)**; until then WT037/039 stay gated. Kept artifacts: `RANGE\administrator` in `SMS Admins`, `SeServiceLogonRight` for `svc_sccm`, `RANGE\svc_sccm` SQL login. New tooling: `sccm-fix-exec-acct.ps1`/`-run.sh` (proper revert). Docs synced: `sccm-integration-guide.md` Phase 6A gate #4, validation report priority 1, `CAMPAIGNS-METADATA-v2.md` (WT035-039 blockquote + WT037/039 rows), branch-c runbook, `CAMPAIGNS_v3.md`, coverage audit.
- **Branch C CD-chain breakthrough — AdminService was NEVER missing (2026-08-01).** Live proof after the SCCM reinstall: the earlier "AdminService NOT deployed" conclusion used IIS-era indicators (IIS app / `SMS_AdminService_AppPool` / `bin\AdminService`) that don't apply to modern builds — since CB 2010 the AdminService is **self-hosted** (`SMS_REST_PROVIDER` inside `SMS_EXECUTIVE`). Verified: `RESTPROVIDERSetup.log` (installed May 23 + re-installed today), `SMS_REST_PROVIDER.log` (started, cert bound), **443 listening**, `/AdminService/v1.0/` → 401 (real auth challenge). **Real blockers found & fixed live:** (1) site reinstall **reset `SMS Admins`** (only mbr02$ + vagrant) → restored `svc_sccm` + `CADRE\chief_command` + `CADRE\analyst_purple`; (2) **CD UAC flag** — playbook set `TRUSTED_FOR_DELEGATION` 0x10000 instead of `TrustedToAuthForDelegation` 0x80000 → S4U2Proxy `KDC_ERR_BADOPTION` → UAC fixed (0x80200 + never-expire) → **ST issued** (getST `administrator@HTTP_mbr02.range.local`); (3) **svc_sccm password expired** → reset + `DONT_EXPIRE_PASSWORD`. **Remaining gate:** REST provider runs as LocalSystem and cannot decrypt the svc_sccm-encrypted ST → 401; must run as `svc_sccm` (self-hosted equivalent of app-pool identity). Playbooks fixed (`05-ad-attack-surface.yml` + `-verifyOnly.yml` add/verify the flag; `10-sccm-verify.yml` checks real indicators). Guide Phase 6A rewritten; validation report/CHECKLIST synced. New tooling: `sccm-review-mbr02-[g..p]`, `cd-chain.cmd`, `cd-present.ps1`, `cd-fix-*.ps1`, `getst-crosscheck.sh`, `sccm-restore-sms-admins*`.
- **WT027 — SPN Jacking (CVE-2026-25177) VERIFIED (2026-08-01).** Last untouched Branch A primitive. **Documented low-priv command NOT viable — real AD behavior surfaced:** (1) `MSSQLSvc/mbr01.child.cadre.local:1433` is owned by `child\svc_mssql` (CN=svc_mssql,OU=Operations) → forest-wide SPN uniqueness → `constraint violation`; (2) a free cross-host service SPN via SELF-write → `Access is denied` (default `Validated write to servicePrincipalName` on SELF allows own-host SPNs only). **Working path verified:** `chief_command` (DA/writeSPN) plants `MSSQLSvc/dc01.cadre.local:14333` on analyst_cloud → LDAP read-back OK → `Rubeus asktgt /enctype:aes256` + `asktgs` → **KDC issued TGS encrypted with analyst_cloud's AES key** (attacker-known → offline crack) → cleanup OK. Quirks recorded: cross-realm SPN → `KDC_ERR_WRONG_REALM` in Rubeus asktgs (same-realm SPN avoids); Server 2025 KDC rejects TGS on RC4 TGT (`KDC_ERR_ETYPE_NOTSUPP` — use AES TGT). New scripts: `04-automation/windows/wt027-spn-jack.ps1`, `-cleanup.ps1`, `-lookup.ps1`, `-lookup-child.ps1`. **All Branch A/B/D attacks now VERIFIED** — Branch A 100%, Branch B (ESC1/2/3/4/7/9 + UnPAC; ESC8/11 deferred), Branch D 100%.
- **Validation matrices synced to verified state (2026-08-01).** The report Appendix A + Top Re-test Priorities and CHECKLIST (summary row, Branch A rows, Re-test priorities, Campaign Re-test Tracker) were STALE — showing ⠿ Scripted / ❌ Missing for attacks verified 07-31/08-01. All synced: only genuinely-open items remain (SCCM WT035-039 pending operator confirm, ESC8/11 deferred, E/F/G/H pending). `split-campaign-runbooks.py --check` unchanged.
- **Campaign folder reorganization (2026-08-01).** Cleaned `attack-matrix/Campaign/` per user direction — only active docs stay at top level (`CAMPAIGNS_v3.md`, `CAMPAIGNS-METADATA-v2.md`, `CAMPAIGNS-VALIDATION-REPORT.md`, `CADRE-Attack-Surface-Coverage-Audit.md`, `Campaign_suggestions.md`, `README.md`, `DFIR-Nexus-Pioneer-workflow.md`, `LAB-PROFILES.md`, `Red-Strike-workflow.md`, `ATTACK-MAP.md`, `attack-tools-required.md`, `.markdownlint.json`, subfolders). Pre-v3 versions moved to **`Campaign/archive/`**: `CAMPAIGNS.md` (v2 index), `CAMPAIGNS_v2.md` (monolith), `CAMPAIGNS_v1_archived.md`, `CAMPAIGNS-METADATA.md`, `CAMPAIGNS-METADATA-mechanics.md`, `Campaign_suggestions-detail.md`, `Feedback_loop.txt`, `_preview/`, `_preview_test_*.html`. Evidence captures → **`Campaign/artifacts/`** (`t025-*.txt`); `patch-validation-report.py` → **`Campaign/automation/`**. ~30 reference sites repointed to v3/-v2/archive: `AGENTS.md` (campaign table + workflow), all runbook headers, `attackpath/`, `01-walkthroughs/` (README + WT028/031), `02-diagrams/`, `prerequisites.md`, `CHECKLIST.md`, `study-guide/`, `Campaign/README.md`, `docs/internal/` (ACTIVE, registry, plan1.1 automation, plan1.7/1.8, external-references). Tools updated: `split-campaign-runbooks.py` (v3 source + archive links in generated headers), `split-campaign-suggestions.py` (detail → `archive/`), `split-campaign-metadata.py` + `render-campaign-preview.py` (operate on archive paths). `python tools/split-campaign-runbooks.py --check` → full coverage OK.
- **GodPotato SYSTEM on mbr01 — VERIFIED (WT041/043).** Full chain via direct `localhost -> ws01` SSH (Rule 1): SQL auth `child\analyst_t1` → `EXECUTE AS LOGIN='sa'` (sysadmin=1) → `xp_cmdshell` (`nt service\mssql$sqlexpress`) → `SeImpersonatePrivilege` → `GodPotato-NET4.exe` → **`nt authority\system`**. Note: WinRM `Copy-Item` to `C:\Users\Public\cadre-gp.exe` denied by stale SYSTEM-owned file — use the `-GpPath` proven path.
- **Branch B — ESC1/ESC3/UnPAC VERIFIED (WT050/051/053).** `certipy req -template CADRE-ESC1 -upn administrator@cadre.local -sid <SID> -dynamic-endpoint` → cert issued → `certipy auth` → PKINIT TGT + **administrator NT hash `81c3b6443f148bf73bb3499791f1eb7b`** (UnPAC). ESC3: agent cert + `-on-behalf-of "cadre\administrator"` (NetBIOS; FQDN form denied by policy `0x80070547`) → admin cert issued, TGT OK (UnPAC on ESC3 cert hit impacket `InvalidChecksum` quirk; UnPAC proven via ESC1 cert). Key learnings: `-sid` required (else SID mismatch), `-dynamic-endpoint` required (default RPC endpoint times out), kill orphaned certipy procs before `certipy auth` (ccache lock hang).
- **ESC8 (WT052) DEFERRED — root cause identified (not a port problem).** Live tcpdump + relay logs proved: (1) the v5 "custom SMB port 8445 / Coercer `@8445` UNC" premise is FALSE — the Windows SMB client does not honor `@port` in UNC (zero connections to non-445 ports); (2) the only working coerce (MS-RPRN) makes the victim dial the attacker's **RPC endpoint 135 anonymously** ("Empty username ... just waiting") — never authenticated SMB to 445; (3) MS-EFSR blocked, MS-DFSNM/MS-FSRVP/MS-EVEN no dial-out on Server 2025. ws01's kernel-held 445 was a red herring. ADCS web-enrollment surface stays configured. Revisit at end (candidates: Kerberos-relay, restoring an SMB-coerce primitive).
- **Branch C SCCM — deep validation (2026-08-01, WT034-039).** SharpSCCM v2.0.13 staged on ws01; SMS Provider (`root\SMS\site_CAD`, site CAD / mbr02.range.local) confirmed reachable from ws01 as `range\svc_sccm` with **explicit credentials over WMI** (no Kerberos needed). **Verified:** svc_sccm is SCCM admin + local `SMS Admins` member; NAA=`RANGE\svc_naa` in machine-policy config; PXE **approved cert** (SMSID `{256B7D4F-…}`, PXE server MBR02) + 2 boot images (x64/arm64); client-push component enabled; **`SMS_Scripts.CreateScripts` works** (caller-generated GUID required); `SMS_Package`+`SMS_Program` (SYSTEM `ProgramFlags=2`) created via provider. **Root-caused constraints:** (1) **AdminService is NOT deployed on mbr02** (no IIS app pool, no `/AdminService` web app in `appcmd list apps`, no `bin\AdminService` dir) → **CMPivot (WT037) / script-run (WT039) have no target**; the `/AdminService/` 401 was IIS-level for a non-existent path, NOT an auth challenge. The `HTTP/mbr02.range.local` SPN on `svc_sccm` + `msDS-AllowedToDelegateTo=HTTP/mbr02.range.local` (CD, intentional per `05-ad-attack-surface.yml`) is **correct**; intended path = Kerberoast → S4U2Self→S4U2Proxy to HTTP/mbr02 → AdminService, with the **AdminService app pool running as `RANGE\svc_sccm`** (SPN owner — required to decrypt the CD ticket). **Fix documented in `docs/sccm-integration-guide.md` Phase 6A (deploy AdminService + pool identity) + verify checks added to `10-sccm-verify.yml`** (web app + files + pool identity). (2) `SMS_Advertisement.Put` = Generic failure via raw WMI (all schedule/token/TimeFlags variants) → app-deploy + full site-takeover exec need console (WT038/039 ⚠️ partial). (3) Script approval `UpdateApprovalState` = Generic failure (deprecated) + `SMS_ScriptsExecutionTask` read-only. (4) **`cifs/mbr02.range.local` SPN MISSING** → SMB Kerberos to mbr02 broken (explains `/ptt`-based SharpSCCM failing); verify-playbook candidate. (5) Cross-forest SCCM admins `CADRE\chief_command` + `analyst_purple` in local `SMS Admins` — finding. Cleanup done (test scripts + package removed); ntlmrelayx listener stopped, smbd restored on provisioning.
- **Branch D Linux — config gaps FIXED + chain VERIFIED.** **Root cause found:** `/etc/krb5.keytab` on linux01 was corrupt/empty → `sssd` failed → no domain-user resolution (SSH/NFS/SSSD all dead). **Fixed live:** recreated machine keytab via `adcli join --domain=cadre.local -U chief_command --stdin-password` (after `rm` of corrupt file) → SSSD active → `getent` resolves `mssql-linux01`. **Second config gap:** no user had sudo/podman → added deliberate sudo misconfig `/etc/sudoers.d/cadre-domain-users` (`mssql-linux01 ALL=(ALL) NOPASSWD:ALL`). **WT047 NFS krb5p fixed too:** registered `nfs/linux01` + `nfs/linux01.cadre.local` SPNs in AD + keytab entries (`adcli update --add-service-principal`), started `rpc-svcgssd` (was FAILED — server GSS), set `/etc/idmapd.conf` Domain=`cadre.local` (was localdomain), and mount **by FQDN** (localhost breaks service-principal). **Proper chain verified:** SSH as `mssql-linux01` → `sudo podman exec cadre-monitor unshare -r id` → **root** (**WT048** ✅); keytab readable (**WT046** ✅); SSSD cache active (**WT045** ✅); **WT047 krb5p mount + read ✅** (write denied by root-owned 0755 share dir). Fixes propagated to **`07-linux-config.yml`** (deploy: keytab ensure, sudo misconfig, idmapd, nfs principal, svcgssd, gssd restarts), **`07-linux-config-verifyOnly.yml`** (7 new checks), and **`sql-integration-guide.md`** (§3.4 machine keytab + pivot access, §3.5 NFS krb5p fixes). Earlier vagrant-based Branch D checks void (config account methodology).
- **Branch B ADCS — ESC2/ESC4/ESC7/ESC9 VERIFIED (2026-08-01).** Fresh `certipy find -vulnerable` confirmed the live surface (CA cadre-CA: ESC7 lead_engineering ManageCA/ManageCertificates, ESC8 HTTP web enrollment, ESC11 ICPR no-encryption; ESC6 not deployed — User Specified SAN disabled; **CADRE-ESC13/14 do NOT exist**). Validated: **ESC2** (CADRE-ESC2 Any Purpose EKU, Req 45) as low-priv `hunter_dfir` → PKINIT TGT as administrator; **ESC9** (CADRE-ESC9 NoSecurityExtension, Req 46) → PKINIT TGT + **UnPAC NT hash `81c3b644…f1eb7b`**; **ESC4** (CADRE-ESC4 WriteDacl) as `lead_engineering` → `certipy template -write-default-configuration` → hunter_dfir enrolled admin cert (Req 47) → NT hash; **template restored** (NameFlag back to original `-1509949440`, EKUs/ACL verified); **ESC7** as `lead_engineering` → `certipy ca -add-officer hunter_dfir` succeeded (ManageCA proof) + removed (cleanup). ESC2 template also flags ESC3+ESC17 (same template). ESC11 deferred with ESC8 (relay family). Test pfx/ccache cleaned from ws01.
- **Corrected doc claims:** the ESC8 v5 "`@8445` UNC resolves" claim in `CAMPAIGNS_v3.md` / `CAMPAIGNS-METADATA-v2.md` / validation report — replaced with the live-tested root cause; validation report statuses updated (GodPotato, ESC1/ESC3/UnPAC ✅, ESC8 🔬 deferred, SCCM surface, Branch D).
- **`attack-matrix/README.md` updated to v3:** START HERE now points to `CAMPAIGNS_v3.md` + `CAMPAIGNS-METADATA-v2.md`; added validation-report + coverage-audit links, the operator-locked hard rules (Rule 1 direct ws01 SSH / Rule 2 no scheduled tasks), updated attack count (107+) and Status table.

## [Unreleased] — 2026-07-31

- Campaign retest pass completed for AD core spine + Branch A/D blockers.
- Verified: direct `localhost -> ws01` SSH via `cadre-ws01-key`; `provisioning` bridge `/tmp/nxc-venv` with `nxc`/`certipy`; Phase 1 AS-REP roast from `provisioning`; Phase 3 SQL `xp_cmdshell` from `ws01`.
- Blocked: Phase 2 Kerberoast (`KDC_ERR_ETYPE_NOSUPP`); Phase 3 GodPotato escalation (`GodPotato.exe` missing on `mbr01`); Phase 3.5A Winlogon extraction and 3.5C RDP (missing execution chain/scripts); Branch A WT015/013/014/016/008/023/024 (missing ACE#7 surface, scripts, or routing).
- Updated: `CAMPAIGNS-VALIDATION-REPORT.md`, `CADRE-Attack-Surface-Coverage-Audit.md`, `CHECKLIST.md`, and credential fixes in `CAMPAIGNS-METADATA-v2.md` / `CAMPAIGNS_v3.md`.
# Changelog
...
**Docs updated:**
- `attack-matrix/Campaign/CADRE-Attack-Surface-Coverage-Audit.md`: updated Branch A and Branch D rows from missing to configured; updated Branch D verdict to reflect playbook coverage.
- `attack-matrix/Campaign/CAMPAIGNS-VALIDATION-REPORT.md`: updated Branch A and Branch D attack status to `📝 Configured / pending re-test` with playbook references and re-test notes.
...
**What was done:**
- Created `docs/internal/plan01-telemetry-catalog/plan1.1-campaign-automation/` (local maintainers) — `README.md` router + `CAMPAIGN-AUTOMATION-PLAN.md` (mirror of RedStrike plan: M1–M5, ws01 routing, HITL gates, Branches A–D/G).
- Updated `docs/internal/PLANS.md` (local maintainers) — Plan **1.1** in naming table, folder table, and execution order (1.1 before/with Plan 1).
- Updated root `CHECKLIST.md` — new **P11.*** section (P11.0 docs done; P11.1–P11.6 pending); top priorities lead with Plan 1.1; `I.1` redirected to P11; companion-doc + NEXT ACTION banner.
- Updated `docs/internal/ACTIVE.md` (local maintainers) — next action = CHECKLIST **P11.1** (RedStrike M1).
