# WT037 CMPivot trigger via SMS_ClientOperation.InitiateClientOperationEx (Type=145) as MBR02\vagrant
# analyst_t1 (ws01 ATTACK) — replicates SharpSCCM fallback
...
# Build the CMPivot script Param (base64 XML) — exact SharpSCCM format
...
Write-Output "[+] Param length=$($Param.Length) for query=$query"
...
Write-Output '[+] Invoking InitiateClientOperationEx (Type=145 CMPivot) ...'
...
Write-Output ("[+] All out params: " + (($out.Properties | ForEach-Object { $_.Name + '=' + $_.Value }) -join ' ; '))
Write-Output ('CMPIVOT_TRIGGER_DONE opId=' + $opId)
