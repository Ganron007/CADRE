...
Write-Output "=== Create SMS_Program (ProgramFlags=2 SYSTEM) ==="
...
$prog["ProgramFlags"] = [uint32]2
$prog["Duration"] = [uint32]5
$prog["Comment"] = "WT039 payload"
try {
    $prog.Put()
    $prog.Get()
    Write-Output "PROG_OK Name=$($prog['ProgramName'])"
} catch {
...
$adv["AssignedSchedule"] = "<RecurrenceInterval Interval=""1"" IsGMT=""false"" />"
try {
    $adv.Put()
    $adv.Get()
