#!/usr/bin/env bash
# T038 — SCCM application deploy check from ws01 (analyst_t1)
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LIB="${SCRIPT_DIR}/../lib"
source "${LIB}/campaign-a-common.sh"

CASE_ID="CADRE-T038-SCCM-APPDEPLOY-${CASE_DATE:-$(date -u +%Y%m%d)}"
T0=$(date -u +%Y-%m-%dT%H:%M:%SZ)

echo "=== T038 SCCM app deploy check | ${CASE_ID} | T0=${T0} ==="

CMD='
$ErrorActionPreference = "Stop";
$pkg = Get-WmiObject -Namespace "root\\ccm\\clientsdk" -Class CCM_Application -ErrorAction SilentlyContinue;
if ($pkg) { Write-Output "T038_OK: CCM_Application query succeeded"; $pkg | Select-Object Name, Id, InstallState -First 5 | Format-Table -AutoSize } else { Write-Output "T038_INFO: no CCM_Application objects" }
Write-Output "T038_OK: app deploy check complete"
'

ws01_exec_as analyst_t1 'T13r_An@lyst!' "$CMD"

cadre_export "${CASE_ID}" T038 "${T0}" 192.168.77.62
echo "T0=${T0}" | tee "/tmp/${CASE_ID}.t0"
echo "=== T038 run complete ==="
