...
| 3.5G | DPAPI via Nemesis (3.5G) | SYSTEM on mbr01 / dc01 | SYSTEM | ✅ VERIFIED 2026-08-03 (end-to-end, CRTE SharpDPAPI) | **Full DPAPI chain on Server 2025:** domain DPAPI backup key EXTRACTED (mimikatz `lsadump::backupkeys /export` on dc01 — RSA PVK + legacy key + DER + PFX) → **CRTE SharpDPAPI (2026-02-02 build) imports the PVK and decrypts cadre masterkeys** (`Administrator` → `{07024635…}:624FE5B4…`, `chief_command` → `{9cdd94c1…}:094F1372…`) → **real blobs decrypted on dc01**: Administrator Credential (`DFBE70A7…` WindowsLive PersistedCredential) + **Web Credentials Vault** (recovered `aes128`/`aes256` vault keys). The `Bad Version of provider` blocker was a **tool-version issue** — the 2026-02-02 build fixes it. | No (verified) |
