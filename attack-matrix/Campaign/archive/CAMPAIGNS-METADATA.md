...
**Companion to** [`CAMPAIGNS_v3.md`](CAMPAIGNS_v3.md). One metadata block per attack.

**Scope:** 81 campaign attacks (main spine Phases 0.5–8 + 4 branches) + 14 E exercises + 10 F supply-chain = **105 total**. E and F are standalone exercises — not part of the campaign narrative.
...
### Mechanics: UnPAC-the-Hash [STUB — UNTESTED] (Plan 1.1 Branch B)

| Field | Value |
|-------|-------|
| **Status** | ⏳ First-class Branch B step (promoted Plan 1.1 M0) |
| **Prereq** | Usable user/machine cert from ESC1/ESC8/etc. |
| **Earns** | NT hash via PKINIT + U2U (no offline crack) |
| **Path** | ws01-primary (Certipy from beachhead or staged tools) |
| **Detection** | 4768/4769 PKINIT/U2U; Certipy LDAP |
| **Ledger** | `produces_cred: nt_hash` for pass-the-hash steps |

**Reproduction checklist:** ESC cert → `certipy auth` → UnPAC/U2U → verify hash works with `nxc smb` → update this section from stub.
