...
> **2026-08-02 FULL EXEC VERIFIED:** WT037 (CMPivot) and WT039 (script-as-SYSTEM) both executed live from ws01 as `range\svc_sccm` against the **WS01 managed client** (ResourceID 16777220). Three real enablers, in order: (1) **BGB fast channel** — `C:\Program Files\SMS_CCM\SMS_BGB` vdir was empty (`/bgb/handler.ashx` → 500) → installed `cd.latest\SMSSETUP\BIN\X64\bgbisapi.msi` + restarted `SMS_EXECUTIVE` → TCP 10123 listening, client signed in; (2) **svc_sccm was never an RBAC admin** (only `MBR02\vagrant` in `RBAC_Admins`) → granted Full Admin via the Takeover-1 DB primitive (`RBAC_Admins` + `RBAC_ExtendedPermissions` mirroring admin 16777217) → Device read 403→200; (3) **script approval** — `UpdateApprovalState` → 500 (author can't self-approve) → `UPDATE Scripts SET ApprovalState=3` (table is `Scripts`). Recipes + full commands in `docs/sccm-integration-guide.md` **Phase 6B**.
