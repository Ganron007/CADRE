...
| WT037 | SCCM CMPivot | ⚠️ Gated — CD auth (AdminService IS up, self-hosted). ST issued (getST) but **401**: provider runs as LocalSystem, can't decrypt ST encrypted to `svc_sccm`. Direct `sc config` change broke the site + was reverted; **only SCCM Setup wizard can run provider as `svc_sccm` (operator action)** → then re-present ST as Administrator | mbr02 | Remote query/exec on managed clients |
...
| WT039 | SCCM Site Takeover | ⚠️ Partial: `SMS_Scripts.CreateScripts` works; approval+run need AdminService (REST provider as `svc_sccm` via Setup wizard, see WT037) or console | mbr02 | Full SCCM site admin → DA in range.local |
