...
| WT037 | SCCM CMPivot | ✅ AdminService auth VERIFIED (2026-08-01) — SPN `HTTP/mbr02.range.local` moved to `mbr02$` (LocalSystem provider decrypts machine-account tickets); svc_sccm keeps decoy `HTTP/sccm.range.local` (WT033); CD unchanged. getST → ST → `AdminService/wmi/SMS_Site` **200** as Administrator (anon 401). CMPivot runnable on MBR02 | mbr02 | Remote query/exec on managed clients |
...
| WT039 | SCCM Site Takeover | ⚠️ Partial — auth gate CLOSED (WT037: AdminService 200 as Administrator); `SMS_Scripts.CreateScripts` works; script approval+run + CMPivot exec on MBR02 = next step (AdminService REST or console) | mbr02 | Full SCCM site admin → DA in range.local |
