...
| 037 | SCCM CMPivot (WT037) | ws01 -> mbr02 | svc_sccm / s3rv1c3_SCCM! | ⚠️ Gated — CD auth (AdminService IS up) | **CORRECTED 2026-08-01:** the earlier "AdminService NOT deployed" claim was wrong (IIS-era indicators). AdminService IS deployed & running (self-hosted `SMS_REST_PROVIDER`; `RESTPROVIDERSetup.log` success; 443 listening; `/AdminService/v1.0/` → 401 auth challenge). Live chain: Kerberoast → S4U2Self ✅ → **S4U2Proxy ❌ `KDC_ERR_BADOPTION`** (root cause: playbook set `TRUSTED_FOR_DELEGATION` 0x10000 instead of `TrustedToAuthForDelegation` 0x80000) → UAC fixed + password reset → **ST issued** (`administrator@HTTP_mbr02.range.local` ccache) → presenting ST → **401** (REST provider runs as LocalSystem, cannot decrypt svc_sccm-encrypted ticket → remaining gate = run the REST provider as `svc_sccm`). Reinstall collateral: SMS Admins reset (restored) + svc_sccm password expired (reset + never-expire). | Yes - after provider identity fix |
