...
| WT037 | SCCM CMPivot | AdminService (`/AdminService`, self-hosted REST provider) | **AdminService IS up** (401 = auth challenge). CD ST issued but **401** — provider runs as LocalSystem, can't decrypt ST encrypted to `svc_sccm`. Direct `sc config` change broke site + reverted. | ⚠️ Gated | Only SCCM Setup wizard (site system installation account) can run provider as `svc_sccm` (operator action) → re-present ST as Administrator. |
...
**Key findings (2026-08-01):** SCCM admin gate = local `SMS Admins` group (svc_sccm + **cross-forest `CADRE\chief_command`/`analyst_purple`**). `svc_sccm` owns `HTTP/mbr02.range.local` SPN + `msDS-AllowedToDelegateTo` (CD) — correct, intended Kerberoast+CD target. AdminService IS deployed (self-hosted). **Remaining gate (WT037/039): REST provider runs as LocalSystem → can't decrypt the CD ST (401, verified). Direct `sc config` account change broke the site (503) and was reverted — only SCCM Setup wizard can run the provider as `svc_sccm` (operator action).** **`cifs/mbr02.range.local` SPN MISSING** → SMB Kerberos to mbr02 broken (NTLM-only; verify-playbook candidate). Site CAD build 9141; 1 device (MBR02); 0 task sequences; 2 boot images.
