...
| WT037 | SCCM CMPivot | AdminService (`/AdminService`, self-hosted REST provider) | **FIXED 2026-08-01:** SPN `HTTP/mbr02.range.local` moved `svc_sccm` → `mbr02$` (LocalSystem provider decrypts machine-account tickets); decoy `HTTP/sccm.range.local` on `svc_sccm` (WT033); CD unchanged. getST → ST → `AdminService/wmi/SMS_Site` **200** as Administrator. | ✅ Auth verified | CMPivot runnable on only managed client (MBR02); full query exec = next step. |
